...
1. 默认使用 `SQLite`，无需额外安装数据库服务。
2. 暂不引入 `Redis`、`Celery` 等中间件。
3. 定时统计和模型训练优先采用 Django 管理命令或离线脚本完成。
4. 部署目标是本地演示和论文实验，不追求高并发生产化架构。
...
- 主模型和对比模型都选择 `scikit-learn` 内可直接使用的轻量方案。
- 不引入额外机器学习服务，不增加额外部署复杂度。
- 训练过程采用离线脚本执行，预测阶段由 Django 直接加载模型文件完成。
...
为了方便开发、展示和论文撰写，项目边界进一步明确如下：

- 保留 `Android + Django + Vue + SQLite` 的完整闭环。
- 保留规则引擎和一个轻量机器学习模型，用于实验对比。
- 保留行为画像、趋势分析、风险解释和预警记录页面。
- 不做复杂分布式部署，不做额外中间件，不做高并发优化。
- 不做深度学习、复杂实时流处理和重型基础设施配置。
